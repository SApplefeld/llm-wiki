...
### Chapter 8 - 2026-07-10
Completed: Finishing pass over Sections 1 through 7 (Section 8, the Eleos pilot, remains open and is Scott's to run)
Implemented By: main session, dispatching qa-verifier, security-reviewer (fable), adversarial-reviewer (fable), docs-curator; one implementer-opus fix round for the security Critical
Metrics: QA 1 round (1 defect, fixed); security 1 round (1 Critical + 2 Major, fixed); adversarial 1 round (1 Critical + 4 Major + 5 Minor, addressed); docs 1 round (2 deviation drift items, fixed); advisor unavailable throughout

This effort stays `In Progress` on purpose. Sections 1 through 7 are Complete, verified, reviewed, and pushed. Section 8 is the live Eleos pilot: it needs Scott's own documents, a real scheduled task registered under his account (nothing has been registered, by design), and his judgment on the wiki's quality. It cannot be done headlessly, so the plan is not archived and its status is not flipped.

QA verification (qa-verifier over all seven sections): one FAIL, now fixed. `Test-KbStructure.ps1` crashed with a raw `Set-StrictMode` exception, rather than a legible `[FAIL]`, on a `settings.json` of `{}`. `Get-JsonProperty` guarded a null object but not a PSCustomObject with zero properties, and projecting `.Name` off an empty property collection throws on both PowerShell 5.1 and 7. Fixed by enumerating the collection one property at a time. All six malformed-settings shapes now report legibly on both shells; the clean template stays green. This is the exact defect class Chapter 1 believed it had fixed, in the one input (`{}`) that distinguishes null from empty, which nobody had tried until QA did.

Security review (whole changeset, fable): one Critical, confirmed by construction and now closed. Claude Code `Bash(cmd:*)` rules are prefix matches with unconstrained arguments, so the allowed `Bash(git diff:*)` and `Bash(git log:*)` were unbounded grants: `git diff --no-index -- /dev/null <path>` prints any file (an arbitrary read that walks around the credential deny rules, since those bind the Read tool, not Bash), and `git diff --output=<path>` / `git log -p --output=<path>` write any file. Independently, a direct experiment showed that `Grep` deny rules with path globs do not bind (a denied decoy was still returned by the Grep tool), while `Read` deny globs do bind. Both holes are the same lesson a third time: a prefix rule over a program with a rich flag surface is unbounded. Fix (Scott's standing "no raw shell verb" model, extended): raw `git` is denied outright, the `Grep` tool is denied outright, and a third helper `kb-status.ps1` exposes exactly the three read-only git queries the skills need (porcelain status, unpushed commits, a page's last-change date) plus the inbox listing, each validating its own arguments. Confirmed in a trusted workspace that the harness itself blocks a denied `git` call ("The system has blocked the `git status` Bash call", no output leaked), so the exfiltration and arbitrary-write primitives are closed at the tool layer, not left to the model's judgment. The credential deny list was broadened (`.claude.json`, `*.pem`/`*.pfx`/`*.key`, `id_rsa`/`id_ed25519`, `.npmrc`/`.pypirc`, docker/kube configs), and every new rule is pinned in the structural test, each watched go red. The residual (an ordinary non-credential file elsewhere on the machine stays readable) is unchanged and remains contained by the one-account-per-entity invariant. The reviewer's blocker assessment: nothing blocks the Eleos pilot now that the Critical is fixed; ASR/NEO rollout was already gated on that fix plus the account boundary.

Adversarial review (whole changeset, fable): the most valuable finding was again a cross-skill seam. An untracked file under `wiki/` (a synthesis page a crashed query wrote but never committed) was recovered by no skill's reconcile step, so `index.md` on the remote could reference a page absent from git. Fixed in all three skills: an untracked `wiki/` page is now recovered work and folds into the `Reconcile:` commit. Verified in isolation: a crashed-query orphan plus a modified `index.md`, empty inbox, produced one `Reconcile:` commit containing both, the page tracked, the tree clean, reached origin. Other findings addressed: the fallback push message now carries an `Ingest:` prefix; the runbook's verification step 2 was overclaiming (an empty-inbox run performs no allow-gated write, so its clean result is identical on a broken setup, and step 3's trust check matched an exact warning string that could change) and now states what it actually proves and greps a stable substring, with step 4's canary named as the decisive proof; `register-task.ps1`'s `RepetitionDuration` of 3650 days (a repeat that silently stops in 2036) is now `[TimeSpan]::MaxValue`; the runbook now requires an elevated session to register a task and names `run-stderr.txt` (added to the template `.gitignore`). The `10 to 15 pages` spec phrasing versus the skill's "however many the source genuinely touches, typically a handful" is a deliberate deviation: a fixed quota against a young ten-page wiki is nonsense, and qualitative "relevant" scales. Recorded here as the deviation the review asked for.

Concurrent-run race, adjudication surfaced to Scott: ingest and lint both edit `wiki/`/`index.md`/`log.md` and each commits pre-existing dirty state at run start, so an ingest firing while a lint is mid-edit can commit and push the lint's half-finished work under a `Reconcile:` subject. State converges and nothing is lost, but a half-written page can reach the remote briefly. This cannot be closed by scheduling alone, because a frequent ingest eventually overlaps a long lint; the proper fix is an mtime gate on reconcile (leave a dirty file alone until untouched for some minutes). Judged out of proportion to the pilot, so the fix is deferred to the backlog and the residual is documented in the runbook with scheduling guidance. Flagged rather than silently built, per the finishing rule.

Docs curation (docs-curator): added `docs/architecture.md` (the operational facts that would otherwise archive away with the plan: the permission model shape, the trust gate, the commit seam, the automation model, the separation boundaries), refreshed the docs index, added the two engine-wide backlog items (out-of-repo read, concurrent-run race), and cross-linked the docs. Two drift items, both `deviation`, both landing on the root `README.md`, which the curator could not write: it still claimed "crash-safe by construction" (Chapter 3 narrowed that to crash-recoverable via reconcile) and implied read isolation was structural (it is enforced by the Windows account boundary, not read scoping). Both fixed by the main session. No `mistake`-class drift.

End-to-end confirmation the main session ran itself, not delegated: a single fresh KB ingested two sources that disagreed on a fact, was linted clean, and answered a query about the disagreement by giving both figures with both citations and refusing to pick a side, explaining that an undated source cannot supersede a dated one. That is the whole thesis of the system working across all three operations on one knowledge base.

State at close: Sections 1 to 7 Complete and pushed. Everything committed. The `s1-wip` stash and the superseded scratch KBs are the only loose ends, named for reaping. The plan stays open for Section 8.
Next: Section 8, the Eleos pilot (Scott, interactive)
Commit Model: Commit-and-Push

### Chapter 9 - 2026-08-18
Completed: Section 8 pre-flight. The three harness assumptions Chapters 1 and 2 confirmed empirically in July were re-verified against the current Claude Code (2.1.234) before the pilot registers a real scheduled task, because five weeks of releases sit between those findings and this run. Done from a session rooted at `D:\`, not in a KB; a throwaway KB (`probe-kb`) was instantiated in the scratchpad with `new-kb.ps1`, left untrusted on purpose, and driven with nested `claude -p` runs. No engine file changed.

Verdicts:

1. **Trust gate: STILL TRUE.** `permissions.allow` is ignored in an untrusted workspace on 2.1.234. Both probe runs printed to stderr: `Ignoring 16 permissions.allow entries from .claude/settings.json: this workspace has not been trusted`, and the message now names both fixes (the interactive trust dialog, or setting `hasTrustDialogAccepted: true`). Workspace trust remains a hard prerequisite of the automation, exactly as Sections 6 and 7 assume, and `register-task.ps1`'s trust refusal is still load-bearing.

2. **Skill invocation form: CHANGED, in our favour.** Chapter 1 found `claude -p "/wiki-ingest"` was read as prose and never invoked the skill. On 2.1.234 it does: both `/wiki-ingest` and `Use the wiki-ingest skill.` fire the Skill tool (`CALL Skill {"skill":"wiki-ingest"}` observed for each), and the slash form ran the whole ingest no-op cleanly. The likely cause is the 2.1.214 changelog fix for bundled skill aliases reporting "Unknown command" in `-p` mode. Consequence for the pilot: none, because `register-task.ps1` uses the prose form, which still works. Consequence for the docs: the "invoke by prose, not slash; the slash form silently no-ops" claim in `README.md`, the skills, and `runbook_automation.md` is now historical rather than current. The prose form is still correct and safe, so this is a doc-accuracy item, not a defect. Left for the pilot session to sweep or for Scott to decide, since changing three shipped docs is out of a re-verify's scope.

3. **Grep deny: STILL EFFECTIVE.** The template denies the `Grep` tool outright (Chapter 8's resolution, after the July finding that path-scoped `Grep(...)` globs do not bind). On 2.1.234 the Grep tool is simply absent from the toolset: the model's `ToolSearch` for Grep returns no match, and it falls back to Bash `grep`. The outright-deny resolution holds, and the deny wall is honored in the untrusted workspace (a `git status` was denied by `Bash(git:*)` in every run).

Two findings the re-verify surfaced that are NOT in any prior Chapter, both observed on this machine (SCOTT-CLAUDE) and both to re-check on the actual pilot account, since the pilot runs under Scott's personal account which may be a different account or box:

4. **This account's user settings default to `bypassPermissions`, and `register-task.ps1` sets no `--permission-mode`.** So a scheduled KB run on an account in this mode would execute under `bypassPermissions`, where the allow-list is no longer a ceiling: everything is permitted except what the `deny` list forbids. The KB security model Chapter 1 rebuilt assumes the allow-list is the ceiling (only the two helper invocations are granted). Under bypass, only the deny wall protects the KB (no raw `git`/`mv`/`rm`/`cp`/`curl`/`wget`, no writes under `.git`/`.claude`/`sources`/`inbox`, no credential-family reads), which still blocks the worst primitives but does not confine writes to `wiki/` or confine Bash to the helpers. The clean untrusted behavior (allow ignored, writes prompt, deny honored) only appeared when the probe forced `--permission-mode default`; without it, on this account, the untrusted Write succeeded under bypass. Decision this raises for Scott, recorded for the pilot: either the pilot account must not run `defaultMode: bypassPermissions`, or `register-task.ps1` should pass `--permission-mode default` so a bypass-defaulted account still runs the KB under its intended allow-list ceiling. The second is a one-line, low-risk hardening of the registrar and is the safer default; it belongs to Section 8 or a small follow-up, not to this re-verify.

5. **`pwsh` (PowerShell 7) is not installed on SCOTT-CLAUDE; only Windows PowerShell 5.1.** The template's allow-list grants only `Bash(pwsh -NoProfile -File ./.claude/kb-*.ps1:*)`, and the skill bodies invoke the helpers as `pwsh -NoProfile -File ...`. With no PS7 present, the agent falls back to `powershell`, which matches no allow rule. In this probe that fallback ran only because the account was in bypass; on a trusted-workspace pilot with the allow-list as the ceiling, every helper call would prompt or be denied and the unattended run would stall. So the template silently assumes PS7 is installed. Options for Scott: install PowerShell 7 on the pilot machine (matches the template as written), or widen the allow-list and the skill invocations to also grant `powershell -NoProfile -File ...` (makes the engine portable to a PS5.1-only box). This is a template portability question, not a machine quirk, and is worth deciding before ASR/NEO rollout onto machines whose PowerShell version is unknown.

Machine facts 4 and 5 are recorded in operator-tier memory (`scott-claude-bypass-permissions-default`, `pwsh-absent-on-scott-claude`) so a future probing or automation session on this box starts from them.
